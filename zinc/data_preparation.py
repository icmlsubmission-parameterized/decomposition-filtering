    ...
    def _prepare(self):
        if os.path.exists(self.pre_processed_file_path):
            ...
            with open(self.data_dir + "/%s.pickle" % self.split, "rb") as f:
                # with open('data/ZINC.pkl', "rb") as f:
                data = pickle.load(f)
            assert len(data) == self.num_graphs, "Sample num_graphs again; available idx: train/val/test => 7985/998/999"
            ...
            print("Filtered graphs: ", self.num_graphs - len(self.graph_lists))
    ...
    def __init__(self, name='Zinc', config=None):
        ...
        if self.name == 'ZINC':
            ...
            self.train = MoleculeAqSolDGL(data_dir, 'train', num_graphs=7985, basis=basis, epsilon=epsilon, power=power,
                                          edgehop=edgehop, degs=degs)
            self.val = MoleculeAqSolDGL(data_dir, 'val', num_graphs=998, basis=basis, epsilon=epsilon, power=power,
                                        edgehop=edgehop, degs=degs)
            self.test = MoleculeAqSolDGL(data_dir, 'test', num_graphs=999, basis=basis, epsilon=epsilon, power=power,
                                         edgehop=edgehop, degs=degs)
